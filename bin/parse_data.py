...
def  getVariancePerRun(infilepath, appGene,appReached,expectedPdr):
	appGenValues = defaultdict(dict) 
	appReachValues = defaultdict(dict)
	col_appGenerated    = None
	col_appReached    = None
	col_cycleNum      = None
	col_runNum      = None
	...
	with open(infilepath,'r') as f:
            for line in f:
                if line.startswith('# '):
                    elems              = re.sub(' +',' ',line[2:]).split()
                    numcols            = len(elems)
                    col_appGenElemName = elems.index(appGene)
                    col_appReachedName = elems.index(appReached)
                    col_cycleNum       = elems.index('cycle')
	...
	with open(infilepath,'r') as f:
		...
		appGenNum    = int(m.group(col_appGenElemName+1))
                appReacNum     =   int(m.group(col_appReachedName+1))

                if (cycleNum)  not in appGenValues or (runNum)  not in appGenValues:
                	appGenValues[runNum][cycleNum]= []
		        appReachValues[runNum][cycleNum] = []
                appGenValues[runNum][cycleNum]= appGenNum
	        appReachValues[runNum][cycleNum] =appReacNum 
       
            unfairnessIndex = []
            varincePerRun = []
            for runNum in range (0,noOfRuns-1):
                    appGenPerRun  = 0 
		    appReacPerRun = 0
		    pPerRun       = []		
		    for cycleNum in  range (0,noOfCycles-1):
		       appGenPerRun  += appGenValues [runNum][cycleNum]  
                       appReacPerRun += appReachValues[runNum][cycleNum]		
                       if cycleNum % 10 == 0:
                           if (appReacPerRun != 0) and  (appGenPerRun !=0) :
				   achievedPDR =  appReacPerRun /  float(appGenPerRun)
		 		   pPerRun.append(achievedPDR / expectedPdr)
                    	   appGenPerRun  = 0
	      	           appReacPerRun = 0
                    if pPerRun:
		    	varincePerRun.append(np.var(pPerRun))
                    else:
                         pPerRun = 0.0
	                 varincePerRun.append(pPerRun) 
              
            unfairnessIndex = np.mean(varincePerRun)   
            print " appGene {0}  unfairness {1} ".format (appGene,unfairnessIndex)    
	return varincePerRun
...
def getData(value, infilepath, outfilepath):
	...
	if value == 'pdr':
		...
		while flow < noOfFlows + 1:
			elemName      = 'appGenerated_F' + str(flow) 
			...
			elemName      = 'appReachesDagroot_F' + str(flow)
			valPerCycle   = getValuesPerCycle(infilepath, elemName)
			avgCycle      = getAverageOfCycle(valPerCycle)
		...
		with open(filename,'w') as f:
			for i in range(noOfCycles-1):
                ...
                plot = "gnuplot -e " + "\"inFilename='" + filename +"';"+ "outFilename='" + filename + ".png';yLabel='PDR'\"" + " " + "plotGraph.gp"      
	...
	elif value == 'avgVariance':
		flow   = 1
		variance_list = []
		while flow < noOfFlows + 1:
			appGene      ='appGenerated_F'+ str(flow)
			appReached   ='appReachesDagroot_F'+str(flow)
                        exptPdrOfFlow   = 'expectedPdrFlow' +str(flow)
                        expectedPdr = getExpectedPdrOfFlow(infilepath,exptPdrOfFlow)
			varince   = getVariancePerRun(infilepath, appGene,appReached,expectedPdr)
			variance_list.append(varince) 
			flow += 1
                	
		filename = outfilepath + "_varince"	  
                runs = range(1, noOfRuns)
		with open(filename,'w') as f:
			for i in range(noOfRuns-1): 
				if noOfFlows == 1: 
					f.write('%d \t%f\n' % (runs[i],  varince_list[0][i]))
				elif noOfFlows == 2: 
					f.write('%d \t%f \t%f\n' % (runs[i], variance_list[0][i],  variance_list[1][i]))
				elif noOfFlows == 3:  
					f.write('%d \t%f \t%f \t%f \n' % (runs[i], variance_list[0][i], variance_list[1][i],variance_list[2][i]))
				else :  
					f.write('%d \t%f \t%f \t%f \t%f\n' % (runs[i], variance_list[0][i], variance_list[1][i],variance_list[2][i],variance_list[3][i]))
                plot = "gnuplot -e " + "\"inFilename='" + filename +"';"+ "outFilename='" + filename + ".png';yLabel='varinace';xLabel='Runs'\"" + " " + "plotGraph.gp"      
		from os import system
		system(plot)

def getExpectedPdrOfFlow (infilepath,flowPdr):
	with open(infilepath,'r') as f:
            for line in f:
                if line.startswith('## '):
	                seachflow = flowPdr + '\s+=\s+([\.0-9]+)'
                	m = re.search(seachflow,line)
                        if m:
                            return float(m.group(1))

def getNoRuns(infilepath):
	with open(infilepath,'r') as f:
            for line in f:
                if line.startswith('## '):
                	m = re.search('numRuns\s+=\s+([\.0-9]+)',line)
                        if m:
...
def main():
	...
	#To Do 
	getData('avgVariance', infilepath, outfilepath)	
