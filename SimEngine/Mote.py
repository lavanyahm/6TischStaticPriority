...
#Propagation Model
	...
	#Schedule updating PDR periodically
    ...
    def _app_action_enqueueData(self,flowId):
	    ...
	    #import pdb;pdb.set_trace()
            appGenerated = "appGenerated" +"_F" +str(flowId)
        ...
        with self.dataLock:
            ...
            if smac:
        	    ...
        	    self._log(self.INFO, "[radio_rxDone] appReachesDagroot @ ASN {0}, Packet {1}", ( self.engine.getAsn(), payload),)           # update mote stats
                    ...
                    #IISc
                    appReachesDagroot = "appReachesDagroot" + "_F" + str(payload[4])
                    self._stats_incrementMoteStats(appReachesDagroot)
    ...
    def _stats_resetMoteStats(self):
        with self.dataLock: 
		 ...
		 droppedMacRetries = "droppedMacRetries_F" + str(i)
                 ...
                 self.motestats[appGenerated]      =        0   # number of packets app layer generated
                 self.motestats[appRelayed]        =        0   # number of packets relayed
                 self.motestats[appReachesDagroot] =        0   # number of packets received at the DAGroot
		 self.motestats[droppedMacRetries] =  0   # packets dropped because more than TSCH_MAXTXRETRIES retries

                 if self.isDelayConstraint == True :
                     droppedDelayExceed = "droppedDelayExceed_F" + str(i) 
                     self.motestats[droppedDelayExceed] = 0    #packets dropped because exceed delay Contraints
